        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if let stages = exercise.stages, let stage = currentStage {
                        ...
                        // NORMAL PATH — existing image + reps/duration + instructions + guide
                        if exercise.imageName.hasPrefix("figure.") {
                            exerciseImageView
                                .frame(height: 300)
                                .clipped()
                        } else {
            ...
            Group {
                if exercise.stages != nil {
                    // Staged exercise — no customization, always close
