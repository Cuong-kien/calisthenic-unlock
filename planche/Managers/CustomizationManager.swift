...
struct CustomizationManager {
    ...
    func effectiveReps(for exercise: Exercise, difficulty: Difficulty? = nil) -> String {
        ...
        if let custom = customization(for: exercise.name, level: exercise.level, difficulty: diff),
           let reps = custom.customReps {
            return "\(reps) rep"
        }
        if let diff = difficulty, exercise.level == .foundation {
    ...
    func setCustomValue(
        ...
        if let existing = customization(for: exerciseName, level: level, difficulty: difficulty) {
